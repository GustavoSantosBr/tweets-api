...
class UpdateTweetsDataset:
    ...
    def __init__(self):
        self.get_tweets = GetTweets()
        self.update_csv_file = UpdateCsvFile()

    def update_tweets_dataset_by_user_name(self, parameters: ParametersDto) -> TweetDatasetDto:
        tweets = self.get_tweets.get_tweets_by_user_name(parameters)
        user_name = parameters.get_name()
        update_file = self.update_csv_file.insert_or_update_file(user_name, tweets.to_table())
